...
deleteBtn.addEventListener('click', function (event) {
    ...
    if (document.getElementById("deleteBtn").value == "Delete") {
        var number = document.getElementById("number").value;
        setVisible(number, 0);
        document.getElementById("deleteBtn").value = "Restore";
    } else {
        var number = document.getElementById("number").value;
        setVisible(number, 1);
...
addBtn.addEventListener('click', function (event) {
    ...
    if (isRealValue(number)) {
        if (checkChanged(number)) {
            updateList(number, start, end, type, detalhes, folderPath, contactID, cliente, now);
        }
    } else {
        addToList(start, end, type, detalhes, folderPath, contactID, cliente, now);
    }
...
function addToList(startdate, enddate, type, details, folderPath, contactID, cliente, now) {
    ...
    // Perform a query
    $query = 'Insert into crm (startdate, enddate, type, details, path, contact, cliente) values ("' + startdate + '","' + enddate + '","' + type + '","' + details + '","' + folderPath + '","' + contactID + '","' + cliente + '","' + now + '")';
...
function updateList(number, startdate, enddate, type, details, folderPath, contactID, cliente, now) {
    console.log("Update");
    var mysql = require('mysql');
    ...
    // Perform a query
    $query = 'UPDATE crm set startdate = "' + startdate + '", enddate= "' + enddate + '",type= "' + type + '", details= "' + details + '", contact= "' + contactID + '", cliente= "' + cliente + '", last_change_date= "' + now + '" Where id = ' + number;
    console.log($query);
    connection.query($query, function (err, rows, fields) {
        if (err) {
            console.log("An error ocurred performing the query.");
...
function checkChanged(id) {
    ...
    con.connect(function (err) {
        if (err) throw err;
        con.query("SELECT last_change_date FROM crm Where id=" + id, function (err, result, fields) {
            if (err) throw err;
            console.log(result);
            var now = Date.now();

            if (open_time < result[0].last_change_date) {

                var timeSinceChange = now - result[0].last_change_date;
                timeSinceChange = timeSinceChange / 1000;
                var mensagem = "Isto foi alterado à " + timeSinceChange + " segundos";
...
function setVisible(id, visible) {
    ...
    // Perform a query
    $query = 'UPDATE crm set visible = "' + visible + '" Where id = "' + id + '";';
    console.log($query);
    connection.query($query, function (err, rows, fields) {
        if (err) {
            console.log("An error ocurred performing the query.");
